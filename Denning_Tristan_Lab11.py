...
zeroes, poles, k = zplane(b, a, filename=None)
...
Phaaaaaaaase, hmagaslealefaslasdfaslkjelkjalskjka = sig.freqz(b, a, whole=True)

plt.figure ( figsize = (10 , 7) )
plt.subplot (2 , 1 , 1)
plt.plot(Phaaaaaaaase/np.pi, 
         20 * np.log10(abs(hmagaslealefaslasdfaslkjelkjalskjka)))
plt.grid ()
plt.ylabel ('|H| [dB]')
plt.title('Task 5')

plt.subplot (2 , 1 , 2)
plt.plot(Phaaaaaaaase/np.pi, np.angle(hmagaslealefaslasdfaslkjelkjalskjka))
plt.grid ()
plt.ylabel(r'$\angle$H($\omega$)')
plt.xlabel(r'$\omega$ [rad/sample]', fontsize = 12)
